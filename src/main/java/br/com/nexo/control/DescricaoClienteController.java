...
public class DescricaoClienteController {
    ...
    public ModelAndView inserir(@Valid DescricaoCliente descricaoCliente, BindingResult bindingResult,
        ...
        DescricaoCliente saved = repDesc.save(descricaoCliente);

        // Chama a IA e salva a predição somente se houver resultado
        Double percentual = predicaoService.obterPercentualMudanca(saved);
        ModelAndView mv = new ModelAndView("descricao_cliente/novo");
        if (percentual != null) {
            Predicao pred = new Predicao();
            pred.setDescricaoCliente(saved);
            pred.setDsResultadoPredicao(percentual.intValue());
    ...
    public ModelAndView historicoFormularios() {
        ...
        java.util.List<DescricaoCliente> descricoes = repDesc.findAll();
